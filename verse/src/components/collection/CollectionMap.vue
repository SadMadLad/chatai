<script setup>
import { createCollectableMap, destroyCollectableMap } from "@/services/apis/collectable_map";
import { watch } from "vue";
import { useCollectableDialogStore } from "@/storage/collectable_dialog";

const { computedCollectableItemId, computedCollectableItemType } = useCollectableDialogStore();
...
watch(collected, async (isCollected) => {
  ...
  if (isCollected)
    await createCollectableMap(payload)
  else
    await destroyCollectableMap(payload)
})
</script>
