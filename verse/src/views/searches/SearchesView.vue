...
<template>
  <h1 class="mx-4 my-12 text-center text-4xl font-black">
    Check Out our AI Search and get recommended resources
  </h1>
  <section class="flex-center">
    <form class="flex flex-row items-center gap-2.5">
      <input
        class="focus:outline-primary-500 h-10 w-80 rounded-lg border border-gray-400 px-2"
        placeholder="Query your search"
        v-model="searchParams.recommendation_search.search_text"
      />
      <button
        @click="handleSubmit"
        :disabled="isLoading || error"
        class="bg-primary-500 hover:bg-primary-400 disabled:bg-primary-200 inline-block rounded-xl px-3.5 py-2.5 font-bold text-white"
      >
        Send
      </button>
    </form>
  ...
  <section v-else class="mx-8">
    <section
      v-if="quizzes && quizzes.isPresent()"
